...
export class TextModelSearch {
	...
	public static findMatches(model: TextModel, searchParams: SearchParams, searchRange: Range, captureMatches: boolean, limitResultCount: number): FindMatch[] {
		// SearchParams contains searchString (helpful in backtracking, find og string)
	...
	private static _doFindMatchesMultiline(model: TextModel, searchRange: Range, searcher: Searcher, captureMatches: boolean, limitResultCount: number): FindMatch[] {
		...
		searcher.reset(0);
		// m is continuously updated to next item in text
		while ((m = searcher.next(text))) {
			// createFindMatch returns FindMatch, a class encapsulating info about a match
			if (counter < 5) {
				counter++
				continue;
			}
			else if (counter >= limitResultCount) {
				return result;
			}
			else {
				result[index++] = createFindMatch(this._getMultilineMatchRange(model, deltaOffset, text, lfCounter, m.index, m[0]), m, captureMatches);
			}
			counter++;
		}
